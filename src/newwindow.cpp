...
newwindow::newwindow(MainWindow *testing) {
  ...
  QVector<double> x(testing->Data_to_fit.size()),
      y(testing->Data_to_fit.size()), yerr(testing->Data_to_fit.size());
  ...
  customPlot->graph(Number_of_Plots)
      ->setScatterStyle(QCPScatterStyle(QCPScatterStyle::ssCircle, 6));
  ...
  customPlot->xAxis->setRange(
      ...
      1.2 *
          (testing->FFunctions[testing->Fit_Order[0]].X_for_Plot
               [(testing->FFunctions[testing->Fit_Order[0]].X_for_Plot.size()) -
                1]));
  customPlot->yAxis->setRange(
      (*std::min_element(
          &(testing->FFunctions[testing->Fit_Order[0]].Y_for_Plot[0]),
          &(testing->FFunctions[testing->Fit_Order[0]]
                .Y_for_Plot[(testing->FFunctions[testing->Fit_Order[0]]
                                 .X_for_Plot.size()) -
                            1]))) *
          0.8,
      (*std::max_element(
  ...
  for (int ijk = 0; ijk < (testing->Number_of_Fits);
    ...
    if (testing->FFunctions[testing->Fit_Order[ijk]].Plot_Fit ==
        true) { // if the checkbox is true (checked)
      QPen pen;
      customPlot->addGraph();
      pen.setWidth(2);
      ...
      customPlot->graph(Number_of_Plots)
          ->setName(QString::fromStdString(
              testing->FFunctions[testing->Fit_Order[ijk]].Function_Name));
      customPlot->graph(Number_of_Plots)->setPen(pen);
      customPlot->graph(Number_of_Plots)
          ->setData(
              QVector<double>(
                  (testing->FFunctions[testing->Fit_Order[ijk]].X_for_Plot)
                      .begin(),
                  (testing->FFunctions[testing->Fit_Order[ijk]].X_for_Plot)
                      .end()),
              QVector<double>(
                  (testing->FFunctions[testing->Fit_Order[ijk]].Y_for_Plot)
                      .begin(),
                  (testing->FFunctions[testing->Fit_Order[ijk]].Y_for_Plot)
                      .end()));
      Number_of_Plots++;
    }
    if (testing->FFunctions[testing->Fit_Order[ijk]].Plot_Confidence_Band ==
        true) {
      for (int j = 0; j < 2; j++) {
        std::vector<double> cb(
            testing->FFunctions[testing->Fit_Order[ijk]].Y_for_Plot);
        for (size_t k = 0;
             k < testing->FFunctions[testing->Fit_Order[ijk]].Y_for_Plot.size();
             k++)
          cb[k] =
              cb[k] +
              (pow(-1, j) *
               testing->FFunctions[testing->Fit_Order[ijk]].Confidence_Band[k]);
        QPen pen;
        pen.setWidth(2);
        ...
        customPlot->graph(Number_of_Plots)
            ->setData(
                QVector<double>(
                    ...
                    (testing->FFunctions[testing->Fit_Order[ijk]].X_for_Plot)
                        .end()),
                QVector<double>(cb.begin(), cb.end()));
...
void newwindow::closeEvent(QCloseEvent *) { writeSettings(); }
