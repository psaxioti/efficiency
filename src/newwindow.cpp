...
//End of plot for the experimental Data graph(0)
   ...
   for ( int ijk=0 ; ijk<(testing->Number_of_Fits) ; ijk++ ) {   // ijk runs for each "theoretical curve
      ...
      if(testing->FFunctions[testing->Fit_Order[ijk]].Plot_Confidence_Band==true) {
         for( int j=0 ; j<2 ; j++ ) {
            ...
            customPlot->addGraph();
            customPlot->graph(Number_of_Plots)->setPen(pen);
            customPlot->graph(Number_of_Plots)->setData(QVector<double>::fromStdVector(testing->FFunctions[testing->Fit_Order[ijk]].X_for_Plot), QVector<double>::fromStdVector(cb));
