...
source ::source(std::string name, double Lt, double Lt_err,
                std::vector<double> en, std::vector<double> en_err,
                std::vector<double> ig, std::vector<double> ig_err) {
  Source_Name = name;
  Lifetime = Lt;
...
fit_function::fit_function(std::string name, std::string picture, int param,
                           double data[], f_function_type f1,
                           df_function_type df1, fdf_function_type fdf1,
                           normal_function ff1, derivative_function ffdf1) {
  Function_Name = name;
  ...
  for (size_t i = 0; i < Number_of_Parameters; i++)
    Parameters_Values[i] = data[i];
  Covariance_Matrix = new double *[Number_of_Parameters];
  for (size_t i = 0; i < Number_of_Parameters; i++)
    Covariance_Matrix[i] = new double[Number_of_Parameters];
  f = f1;
  df = df1;
  fdf = fdf1;
  Fit_Function = ff1;
...
fit_function::fit_function(std::string name, std::string picture, int param,
                           double data[], normal_function f1,
                           derivative_function df1) {
  Function_Name = name;
  Function_Picture = picture;
  Number_of_Parameters = param;
  Parameters_Values = new double[Number_of_Parameters];
...
int fit_function::fdf1(const gsl_vector *x, void *params, gsl_vector *f,
                       gsl_matrix *J) {
...
std::vector<source> create_sources() {
  ...
  if (!sources.is_open()) {
    QString fileName =
        QFileDialog::getOpenFileName(0, "Open file with sources");
  ...
  while (!sources.eof()) {
    ...
    if (c == '#' || c == '\n' || c == EOF) {
      sources.ignore(512, '\n');
      if (energies.size() > 0.) {
        source test1(Source_Name, Lifetime, Lifetime_error, energies,
                     energies_err, Intensity, Intensity_err);
...
std::vector<fit_function> create_functions() {
...

  {
    ...
    int Debertin_fdf(const gsl_vector *x, void *params, gsl_vector *f,
                     gsl_matrix *J);
    ...
    fit_function test1(name, picture, no_parameters, xinit, f, df, fdf, ff,
                       ffdf1);
    test.push_back(test1);
  }
  {
    ...
    int IAEA_fdf(const gsl_vector *x, void *params, gsl_vector *f,
                 gsl_matrix *J);
  ...
  }
  {
    ...
    int Kalfas_fdf(const gsl_vector *x, void *params, gsl_vector *f,
                   gsl_matrix *J);
...
double Debertin(double x, double *param) {
  double lnt = log(x);
  double Yi =
      param[0] * lnt + (param[1] * lnt + param[2] * pow(lnt, 2.) +
                        param[3] * pow(lnt, 4.) + param[4] * pow(lnt, 5.)) /
                           x;
  return Yi;
}
...
double Debertin_Derivative(int i, double x, double *) {
  double lnt = log(x);
  double Yi = 0.;
  if (i == 0)
    Yi = lnt;
...
int Debertin_f(const gsl_vector *x, void *params, gsl_vector *f) {
  ...
  for (i = 0; i < n; i++) {
    double lnt = log(t[i]);
    double Yi = A * lnt + 1. / t[i] *
                              (B * lnt + C * pow(lnt, 2.) + D * pow(lnt, 4.) +
                               E * pow(lnt, 5.));
    gsl_vector_set(f, i, (Yi - yy[i]) / sigma[i]);
  }
  return GSL_SUCCESS;
...
int Debertin_df(const gsl_vector *, void *params, gsl_matrix *J) {
  size_t n = ((struct data *)params)->n;
  double *t = ((struct data *)params)->t;
  double *sigma = ((struct data *)params)->sigma;
...
double IAEA(double x, double *param) {
  double Yi =
      param[0] + param[1] / x + param[2] / pow(x, 2.0) + param[3] / pow(x, 3.0);
  return Yi;
}

///////////////////////////////////////////////////////////////////////

double IAEA_Derivative(int i, double x, double *) {
  double Yi = 0.;
  if (i == 0)
    Yi = 1;
  else if (i == 1)
...
int IAEA_df(const gsl_vector *, void *params, gsl_matrix *J) {
  size_t n = ((struct data *)params)->n;
  double *t = ((struct data *)params)->t;
  double *sigma = ((struct data *)params)->sigma;
...
double Kalfas(double x, double *param) {
  double Yi =
      param[0] * pow(x, param[1]) / (1000 * param[2] + pow(x, param[3]));
  return Yi;
}
...
double Kalfas_Derivative(int i, double x, double *param) {
  double Yi = 0.;
  if (i == 0)
    Yi = pow(x, param[1]) / (1000 * param[2] + pow(x, param[3]));
  else if (i == 1)
    Yi = param[0] * pow(x, param[1]) * log(x) /
         (1000 * param[2] + pow(x, param[3]));
  else if (i == 2)
    Yi = -1000 * param[0] * pow(x, param[1]) /
         pow((1000 * param[2] + pow(x, param[3])), 2);
  else if (i == 3)
    Yi = -param[0] * pow(x, param[1]) * pow(x, param[3]) * log(x) /
         pow(1000 * param[2] + pow(x, param[3]), 2);
  return Yi;
}
...
int Kalfas_df(const gsl_vector *x, void *params, gsl_matrix *J) {
  ...
  for (i = 0; i < n; i++) {
    ...
    gsl_matrix_set(
        J, i, 1, A * pow(ener, B) * log(ener) / (1000 * C + pow(ener, D)) / s);
    gsl_matrix_set(J, i, 2,
                   -1000 * A * pow(ener, B) /
                       pow((1000 * C + pow(ener, D)), 2) / s);
    gsl_matrix_set(J, i, 3,
                   -A * pow(ener, B) * pow(ener, D) * log(ener) /
                       pow(1000 * C + pow(ener, D), 2) / s);
  }
  return GSL_SUCCESS;
}
